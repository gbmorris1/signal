import Svg, { Path } from 'react-native-svg';
import type { Platform } from '@/types';

// Official brand marks, traced from each platform's own logo (nominative use —
// these mark the "Open on …" links out to the real venues). Polymarket's
// bowtie renders as a filled evenodd path (the hollows come from the fill
// rule); Kalshi's mark is its "K" glyph.
const POLYMARKET_PATH =
  'M136.267 152.495C136.267 159.76 136.267 163.392 133.891 165.192C131.516 166.993 128.019 166.012 121.024 164.049L8.63192 132.51C4.41793 131.328 2.31093 130.737 1.09248 129.129C-0.125977 127.522 -0.125977 125.333 -0.125977 120.957V47.0434C-0.125977 42.6667 -0.125977 40.4783 1.09248 38.8709C2.31093 37.2634 4.41792 36.6722 8.63191 35.4897L121.024 3.95096C128.019 1.98834 131.516 1.00703 133.891 2.80771C136.267 4.60839 136.267 8.24049 136.267 15.5047V152.495ZM27.9043 122.228L120.966 148.345V96.1133L27.9043 122.228ZM15.1738 110.111L108.217 84L15.1738 57.8887V110.111ZM27.9033 45.7725L120.966 71.8877V19.6553L27.9033 45.7725Z';
const KALSHI_PATH =
  'M105.23 105.628L179.66 222.61H115.118L54.3009 121.934V222.61H0V3.38607H54.3009V99.102L119.464 3.38607H177.489L105.23 105.628Z';

// Intrinsic aspect ratios (width / height) so the mark stays proportional
// when we size by height.
const RATIO: Record<Platform, number> = {
  polymarket: 138 / 168,
  kalshi: 180 / 226,
};

/** Renders the platform's brand mark at a given height, in a single color. */
export function PlatformLogo({
  platform,
  size = 16,
  color,
}: {
  platform: Platform;
  size?: number;
  color: string;
}) {
  const width = size * RATIO[platform];
  if (platform === 'polymarket') {
    return (
      <Svg width={width} height={size} viewBox="-1 0 138 168" fill="none">
        <Path fillRule="evenodd" clipRule="evenodd" d={POLYMARKET_PATH} fill={color} />
      </Svg>
    );
  }
  return (
    <Svg width={width} height={size} viewBox="0 0 180 226" fill="none">
      <Path d={KALSHI_PATH} fill={color} />
    </Svg>
  );
}
